...
<?php if (ot_get_option('disablepageendbox') == 'no') { ?>
		...
		<aside id="endpage-box" class="hide-on-print" style="border-color: <?php echo $color ?>;">
			...
			<?php while ($query->have_posts()) : $query->the_post(); ?>
				<aside style="border-color: <?php echo $color ?>;"><a href="<?php echo $categorylink; ?>" style="color: <?php echo $color ?>;"><?php _e( ''.$categoryname, THB_THEME_NAME ); ?></a></aside>
				<h3><?php the_title(); ?></h3>
				<a href="<?php the_permalink(); ?>" class="btn more-link"><?php _e( 'DEVAMI', THB_THEME_NAME ); ?></a>
			<?php endwhile; ?>
		</aside>
		<?php endif; ?>
		<?php wp_reset_query(); ?>
