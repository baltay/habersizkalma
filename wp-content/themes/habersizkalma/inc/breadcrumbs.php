...
function thb_breadcrumb() {
	...
	echo '<li><a href="';
	echo home_url();
	echo '">'.__('Ana Sayfa', THB_THEME_NAME);
	echo "</a></li>";
	}
...
function thb_which_archive() {
	...
	elseif (is_day())
	{
		...
		$output = get_the_time('F jS, Y')." tarihli haberler";
	}
	elseif (is_month())
	{
		$output = get_the_time('F, Y')." tarihli haberler";
	}
	elseif (is_year())
	{
		$output = get_the_time('Y')." yılına ait haberler";
	}
	elseif (is_search())
	{
		global $wp_query;
		if(!empty($wp_query->found_posts))
		{
			...
			{
				$output =  esc_attr( get_search_query() ) . " için arama sonuçları";
			}
			else
			{
				$output =  esc_attr( get_search_query() ) . " için arama sonuçları";
	...
	elseif (is_author())
	{
		...
		$output = __('', THB_THEME_NAME)." ";

		if(isset($curauth->nickname)) $output .= $curauth->user_nicename;

	}
	elseif (is_tag())
	{
		$output = single_tag_title('',false)." etiketi için haberler";
	}
	elseif(is_tax())
	{
		$term = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) );
		$output = $term->name." için haberler";
	}
	else
	{
		$output = __('Haberler', THB_THEME_NAME)." ";
	}
